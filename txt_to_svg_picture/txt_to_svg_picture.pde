...
// Milling
static float milling_waveform_amplitude_pixels = 100.0;
//static int milling_width_in = 36;
...
void setup () {
  ...
  println("Screen resolution in dpi: " + dpi);     
  //assert(width_in == 36);
  //assert(height_in == 24);
  //size(3456, 2304);
  assert(width_in == 60);
...
void draw() {
  ...
  if (actual_vertices_drawn % 2000 == 0 && song_data_index != 0) {
    ...
    if (split_pdfs && actual_vertices_drawn % 6000 == 0 && song_data_index != 0) {
